                ...
                if($myresult->num_rows > 0){
					...
					$myresult2 = $db->query($myquery2);
						if($myresult2->num_rows > 0){
							...
							"', pincode = '".$pincode."', phone = '".$phone."', ownership_type = '".$ownership_type."' WHERE email = '".$email."' AND password = '".$password2."'");
						}else{
							$db->query("INSERT INTO '".$mysql_tb2."' (personal_name, loc_owner_name, address, country, state, city, pincode, 
							email, phone, password, ownership_type) VALUES ('".
							$personal_name."','".$loc_owner_name."','".$address."','".$country."','".$state."','".$city.
							"','".$pincode."','".$email."','".$phone."','".$password2."','".$ownership_type."')");
						}
                }else{
